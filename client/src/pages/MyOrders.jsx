...
const MyOrders = () => {
    ...
    const confirmCancelOrder = async () => {
        ...
        try {
            const { data } = await axios.patch(`/api/order/${orderIdToCancel}/cancel`)
            if (data.success) {
    ...
    return (
        <div className="mt-8 md:mt-12 pb-14 max-w-2xl mx-auto w-full text-sm">
                            ...
                            >
                                <div className="flex flex-col gap-3 px-4 py-3.5 sm:flex-row sm:items-center sm:justify-between sm:gap-4 border-b border-neutral-100">
                                    ...
                                    <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                                        ...
                                        {fulfillment?.toLowerCase() !== 'cancelled' && fulfillment?.toLowerCase() !== 'canceled' && (
                                            <>
                                        ...
                                        {fulfillment?.toLowerCase() !== 'cancelled' && fulfillment?.toLowerCase() !== 'canceled' && fulfillment?.toLowerCase() !== 'delivered' && (
                                            <button
                                                onClick={() => handleCancelClick(order._id)}
                                                className="px-2 py-1 ml-2 cursor-pointer rounded text-xs font-medium text-rose-600 bg-rose-50 hover:bg-rose-100 transition"
                                            >
                                                Cancel
                                            </button>
                                        )}
            ...
            {cancelConfirm && (
                <div className="fixed inset-0 bg-gray-600/20 backdrop-blur-md flex items-center justify-center z-[100] transition-all p-4">
                    <div className="bg-white rounded-2xl p-6 shadow-2xl max-w-sm w-full border border-gray-100 animate-in fade-in zoom-in duration-200">
                        <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-rose-50 border border-rose-100 mx-auto sm:mx-0">
                             <svg className="w-6 h-6 text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                             </svg>
                        </div>
                        <h3 className="text-xl font-bold text-gray-900 mb-2 text-center sm:text-left">Cancel Order?</h3>
                        <p className="text-gray-600 mb-6 text-center sm:text-left leading-relaxed">
                            Are you sure you want to cancel this order? This action cannot be undone once processed.
                        </p>
                        <div className="flex gap-3 justify-center sm:justify-end">
                            <button 
                                onClick={abortCancel}
                                className="flex-1 sm:flex-none px-6 py-2.5 bg-gray-50 text-gray-700 rounded-xl font-semibold hover:bg-gray-100 transition-all cursor-pointer border border-gray-200"
                            >
                                No, Keep it
                            </button>
                            <button 
                                onClick={confirmCancelOrder}
                                className="flex-1 sm:flex-none px-6 py-2.5 bg-rose-500 text-white rounded-xl font-semibold hover:bg-rose-600 transition-all shadow-lg shadow-rose-200 cursor-pointer"
                            >
                                Yes, Cancel
                            </button>
                        </div>
